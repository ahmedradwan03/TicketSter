...
export default async function Matches() {
    ...
    return (
        <div className="w-[80%] mx-auto p-8 space-y-8">
            <CreateMatchForm teams={teamsResponse.teams} stadiums={stadiumsResponse.stadiums} />

            {matchesResponse?.matches.map((match: MatchDto) => (
                <UpdateMatchForm key={match.id} match={match} teams={teamsResponse.teams} stadiums={stadiumsResponse.stadiums} />
            ))}
        </div>
    );
}
